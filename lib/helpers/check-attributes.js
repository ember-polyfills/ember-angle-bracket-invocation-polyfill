...
module.exports = function checkAttributes(node, supported) {
  ...
  if (node.modifiers.length > 0) {
    throw new SilentError(
      `Passing element modifiers to the <${node.tag}> component at ${sourceReference} is not supported by ember-angle-bracket-invocation-polyfill.`
    );
  }
  ...
  if (unsupported.length > 0) {
    console.warn(
      `You passed ${
        ...
        node.tag
      }> component at ${sourceReference} which cannot be supported by ember-angle-bracket-invocation-polyfill: ${unsupported.join(
        ', '
      )}`
    );
  }
